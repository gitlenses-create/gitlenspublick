/* GitLens — chart rendering (donut + line), adapted from the original _charts logic */
...
function donutChart(objectName, data) {
    ...
    if (objectName === "repoCommitCount") {
        tooltipInfo = data[objectName + "Descriptions"];
        arrayRotate(colors, 4);
    }
    if (objectName === "repoStarCount") {
        tooltipInfo = data[objectName + "Descriptions"];
        arrayRotate(colors, 2);
    }
